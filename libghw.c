...
__attribute__((noreturn)) static void ghw_error_exit_line(char const *file,
                                                          int line) {
  fprintf(stderr,
          "Failed to load ghw file due to invalid data. Terminating.\n");
...
// Call malloc (size), fail exit when malloc fails (returns NULL)
void *malloc_unwrap(size_t size) {
  void *ret = malloc(size);
  if (ret == NULL) {
    fprintf(stderr, "libghw could not allocate %zu bytes. Terminating.\n",
            size);
    exit(12);
  }
  return ret;
...
// Call calloc ()
void *calloc_unwrap(size_t nmemb, size_t size) {
  void *ret = calloc(nmemb, size);
  if (ret == NULL) {
    fprintf(stderr,
            "libghw could not allocate %zu elements of size %zu bytes. "
            "Terminating.\n",
            nmemb, size);
    exit(12);
  }
...
// Read an i32, make sure it's positive, cast to u32
uint32_t ghw_get_i32_positive(struct ghw_handler *h, unsigned char *b) {
  int32_t val_i = ghw_get_i32(h, b);
  if (val_i < 0)
    ghw_error_exit();
...
// Get the string with id str_id from h->str_table if the ID is valid.
static const char *ghw_get_str(struct ghw_handler *h, uint32_t str_id) {
  if (str_id >= h->nbr_str) {
    fprintf(stderr, "Invalid string ID %u in ghw file\n", str_id);
    ghw_error_exit();
  }
  return h->str_table[str_id];
}

// Read a string ID from the input file and return the correpsonding string.
const char *ghw_read_strid(struct ghw_handler *h) {
  uint32_t id;

  if (ghw_read_uleb128(h, &id) != 0)
    ghw_error_exit();
  return ghw_get_str(h, id);
}

// Get the type with ID type_id from h->types if the ID is valid and
// the type has been initialized.
union ghw_type *ghw_get_typeid(struct ghw_handler *h, uint32_t type_id) {
  if ((type_id < 1) || type_id - 1 >= h->nbr_types ||
      h->types[type_id - 1] == NULL) {
    fprintf(stderr, "Invalid typeid ID %u in ghw file\n", type_id);
    ghw_error_exit();
  }
  return h->types[type_id - 1];
}

// Read a typeid ID from the input file and return the correpsonding string.
union ghw_type *ghw_read_typeid(struct ghw_handler *h) {
  uint32_t id;

  if (ghw_read_uleb128(h, &id) != 0)
    ghw_error_exit();
  return ghw_get_typeid(h, id);
}

union ghw_range *ghw_read_range(struct ghw_handler *h) {
  ...
  case ghdl_rtik_type_b2: {
    ...
    r = malloc_unwrap(sizeof(struct ghw_range_b2));
    r->kind = t & 0x7f;
    r->dir = (t & 0x80) != 0;
  ...
  case ghdl_rtik_type_e8: {
    ...
    r = malloc_unwrap(sizeof(struct ghw_range_e8));
    r->kind = t & 0x7f;
    r->dir = (t & 0x80) != 0;
    if (ghw_read_byte(h, &r->left) != 0)
      goto err_e8;
  ...
  case ghdl_rtik_type_p32: {
    ...
    r = malloc_unwrap(sizeof(struct ghw_range_i32));
    r->kind = t & 0x7f;
    r->dir = (t & 0x80) != 0;
  ...
  case ghdl_rtik_type_p64: {
    ...
    r = malloc_unwrap(sizeof(struct ghw_range_i64));
    r->kind = t & 0x7f;
    r->dir = (t & 0x80) != 0;
  ...
  case ghdl_rtik_type_f64: {
    ...
    r = malloc_unwrap(sizeof(struct ghw_range_f64));
    r->kind = t & 0x7f;
    r->dir = (t & 0x80) != 0;
...
int ghw_read_str(struct ghw_handler *h) {
  ...
  // Read number of strings and total string size. Make sure that nbr_string
  // is not too large, as it's incremented afterwards.
  h->nbr_str = ghw_get_i32_positive(h, &hdr[4]);
  h->nbr_str++;
  h->str_size = (uint32_t)ghw_get_i32(h, &hdr[8]);

  // Allocate str_table, str_content. Avoid exceeding size_t limits.
  h->str_table = (char **)calloc_unwrap(h->nbr_str, sizeof(char *));

  uint64_t alloc_size = h->str_size + h->nbr_str + 1;
  if (alloc_size > SIZE_MAX)
    ghw_error_exit();
  h->str_content = (char *)malloc_unwrap(alloc_size);
  char *p_end = h->str_content + alloc_size;
...
err_ghw_read_str:
  fprintf(stderr, "Invalid string entry in GHW file.\n");
  ghw_error_exit();
}
...
static int get_nbr_elements(union ghw_type *t) {
  switch (t->kind) {
  case ghdl_rtik_type_b2:
  case ghdl_rtik_type_e8:
  case ghdl_rtik_type_e32:
...
struct ghw_subtype_array *ghw_read_array_subtype(struct ghw_handler *h,
                                                 union ghw_type *base) {
  struct ghw_type_array *arr = (struct ghw_type_array *)ghw_get_base_type(base);
  if (arr->kind != ghdl_rtik_type_array)
    ghw_error_exit();
  ...
  sa = malloc_unwrap(sizeof(struct ghw_subtype_array));
  sa->kind = ghdl_rtik_subtype_array;
  sa->name = NULL;
  sa->base = base;
  nbr_els = get_nbr_elements(arr->el);
  nbr_scalars = 1;
  sa->rngs = calloc_unwrap(arr->nbr_dim, sizeof(union ghw_range *));
...
ghw_read_record_subtype(struct ghw_handler *h, struct ghw_type_record *base) {
  ...
  sr = malloc_unwrap(sizeof(struct ghw_subtype_record));
  ...
  if (base->nbr_scalars >= 0) {
    ...
    sr->els =
        calloc_unwrap(base->nbr_fields, sizeof(struct ghw_record_element));
    nbr_scalars = 0;
...
int ghw_read_type(struct ghw_handler *h) {
  ...
  h->nbr_types = ghw_get_i32_positive(h, &hdr[4]);

  // Allocate type storage, initialize it to 0 to catch invalid accesses.
  h->types =
      (union ghw_type **)calloc_unwrap(h->nbr_types, sizeof(union ghw_type *));
  ...
  for (i = 0; i < h->nbr_types; i++) {
    ...
    case ghdl_rtik_type_e8: {
      ...
      e = malloc_unwrap(sizeof(struct ghw_type_enum));
      e->lits = NULL;
      e->kind = t;
      ...
      if (ghw_read_uleb128(h, &e->nbr) != 0)
        goto err_rt_b2;
      uint64_t lit_size = e->nbr * sizeof(char *);
      if (lit_size > SIZE_MAX)
        ghw_error_exit();
      e->lits = (const char **)malloc_unwrap(lit_size);
      if (h->flag_verbose > 1)
    ...
    case ghdl_rtik_type_f64: {
      ...
      sc = malloc_unwrap(sizeof(struct ghw_type_scalar));
      sc->kind = t;
    ...
    case ghdl_rtik_type_p64: {
      ...
      ph = malloc_unwrap(sizeof(struct ghw_type_physical));
      ph->kind = t;
      ...
      if (h->version == 0)
        ...
        if (ghw_read_uleb128(h, &ph->nbr_units) != 0)
          goto err_p32;
        ph->units = calloc_unwrap(ph->nbr_units, sizeof(struct ghw_unit));
    ...
    case ghdl_rtik_subtype_scalar: {
      ...
      ss = malloc_unwrap(sizeof(struct ghw_subtype_scalar));
      ss->kind = t;
    ...
    case ghdl_rtik_type_array: {
      ...
      arr = malloc_unwrap(sizeof(struct ghw_type_array));
      arr->dims = NULL;
      arr->kind = t;
      ...
      if (ghw_read_uleb128(h, &arr->nbr_dim) != 0)
        goto err_array;
      arr->dims = (union ghw_type **)calloc_unwrap(arr->nbr_dim,
    ...
    case ghdl_rtik_subtype_unbounded_array: {
      ...
      sua = malloc_unwrap(sizeof(struct ghw_subtype_unbounded_array));
      sua->kind = t;
    ...
    case ghdl_rtik_type_record: {
      ...
      rec = malloc_unwrap(sizeof(struct ghw_type_record));
      rec->kind = t;
    ...
    case ghdl_rtik_subtype_record: {
      ...
      name = ghw_read_strid(h);
      union ghw_type *base_u = ghw_read_typeid(h);
      if (base_u->kind != ghdl_rtik_type_record)
        ghw_error_exit();
      base = (struct ghw_type_record *)base_u;
    ...
    case ghdl_rtik_subtype_unbounded_record: {
      ...
      sur = malloc_unwrap(sizeof(struct ghw_subtype_unbounded_record));
      sur->kind = t;
      sur->name = ghw_read_strid(h);
      union ghw_type *base_u = ghw_read_typeid(h);
      if (base_u->kind != ghdl_rtik_type_record)
        ghw_error_exit();
      sur->base = (struct ghw_type_record *)base_u;
...
void ghw_disp_typename(struct ghw_handler *h, union ghw_type *t) {
  (void)h;
...
int ghw_read_signal(struct ghw_handler *h, unsigned int *sigs,
                    union ghw_type *t) {
  switch (t->kind) {
  case ghdl_rtik_type_b2:
  case ghdl_rtik_type_e8:
...
int ghw_read_value(struct ghw_handler *h, union ghw_val *val,
                   union ghw_type *type) {
...
int ghw_read_hie(struct ghw_handler *h) {
  ...
  blk = (struct ghw_hie *)malloc_unwrap(sizeof(struct ghw_hie));
  ...
  h->sigs_no_null = 0;
  h->sigs =
      (struct ghw_sig *)calloc_unwrap(h->nbr_sigs, sizeof(struct ghw_sig));
  ...
  while (1) {
    ...
    el = (struct ghw_hie *)malloc_unwrap(sizeof(struct ghw_hie));
    el->kind = t;
    el->parent = blk;
    el->brother = NULL;
    ...
    *last = el;
    ...
    case ghw_hie_package:
      ...
      if (t == ghw_hie_generate_for) {
        ...
        el->u.blk.iter_value = malloc_unwrap(sizeof(union ghw_val));
        if (ghw_read_value(h, el->u.blk.iter_value, el->u.blk.iter_type) < 0)
          return -1;
      }
      blk = el;
      last = &el->u.blk.child;
    ...
    case ghw_hie_port_linkage:
      /* For a signal, read type.  */
      {
        ...
        if (nbr_el < 0)
          return -1;
        sigs = (unsigned int *)calloc_unwrap(nbr_el + 1, sizeof(unsigned int));
        el->u.sig.sigs = sigs;
  ...
  for (i = 0; i < h->nbr_sigs; i++)
    if (h->sigs[i].type != NULL)
      h->sigs[i].val = (union ghw_val *)malloc_unwrap(sizeof(union ghw_val));
    else if (i > 0 && sigs_no_null != 0) {
      printf("Warning: ghw_read_hie: NULL type signal %ud.\n", i);
...
static void print_name(struct ghw_hie *hie, int full_names) {
  int i;
  int depth;
  struct ghw_hie *p;
  struct ghw_hie **buf;
...
int ghw_read_signal_value(struct ghw_handler *h, struct ghw_sig *s) {
  if (s->type == NULL)
    ghw_error_exit();
...
static const char *ghw_get_lit(union ghw_type *type, uint32_t e) {
  if (e >= type->en.nbr)
    return "??";
  else
...
static void ghw_disp_lit(union ghw_type *type, uint32_t e) {
...
void ghw_get_value(char *buf, int len, union ghw_val *val,
  ...
  case ghdl_rtik_type_b2:
    if (val->b2 <= 1) {
      strncpy(buf, ghw_get_lit(base, val->b2), len - 1);
      buf[len - 1] = 0;
    } else {
  ...
  case ghdl_rtik_type_e8:
    if (val->b2 <= base->en.nbr) {
      strncpy(buf, ghw_get_lit(base, val->e8), len - 1);
      buf[len - 1] = 0;
    } else {
...
static char is_skip_signal(int *signals_to_keep, int nb_signals_to_keep,
                           int signal) {
  int i;
...
void ghw_filter_signals(struct ghw_handler *h, int *signals_to_keep,
  ...
  if (0 < nb_signals_to_keep && 0 != signals_to_keep) {
    ...
    for (i = 0; i < h->nbr_sigs; ++i) {
      h->skip_sigs[i] = is_skip_signal(signals_to_keep, nb_signals_to_keep, i);
    }
  } else {
    if (0 != h->skip_sigs) {
      free(h->skip_sigs);
...
const char *ghw_get_dir(int is_downto) { return is_downto ? "downto" : "to"; }
...
void ghw_disp_range(union ghw_type *type, union ghw_range *rng) {
  ...
  case ghdl_rtik_type_i32:
  case ghdl_rtik_type_p32:
    printf(GHWPRI32 " %s " GHWPRI32, rng->i32.left, ghw_get_dir(rng->i32.dir),
           rng->i32.right);
    break;
  case ghdl_rtik_type_i64:
  case ghdl_rtik_type_p64:
    printf(GHWPRI64 " %s " GHWPRI64, rng->i64.left, ghw_get_dir(rng->i64.dir),
           rng->i64.right);
    break;
  case ghdl_rtik_type_f64:
...
static void ghw_disp_array_subtype_bounds(struct ghw_subtype_array *a) {
  unsigned i;
...
static void ghw_disp_record_subtype_bounds(struct ghw_subtype_record *sr) {
...
static void ghw_disp_subtype_definition(struct ghw_handler *h,
                                        union ghw_type *t) {
  switch (t->kind) {
  case ghdl_rtik_subtype_scalar: {
...
static int ghw_is_anonymous_type(struct ghw_handler *h, union ghw_type *t) {
...
void ghw_disp_types(struct ghw_handler *h) {
  ...
  for (i = 0; i < h->nbr_types; i++)
    if (h->types[i] != NULL &&
        (h->flag_verbose || !ghw_is_anonymous_type(h, h->types[i])))
      ghw_disp_type(h, ghw_get_typeid(h, i + 1)); // pass i+1 to access index i
}
